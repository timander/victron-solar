...
class SolarRecord(BaseModel):
    ...
    yield_wh: float = Field(..., alias="Yield(Wh)", ge=0, description="Daily solar yield in watt-hours")
    consumption_wh: float = Field(..., alias="Consumption(Wh)", ge=0, description="Daily consumption in watt-hours")
    max_pv_power_w: float = Field(..., alias="Max. PV power(W)", ge=0, description="Maximum PV power in watts")
    max_pv_voltage_v: float = Field(..., alias="Max. PV voltage(V)", ge=0, description="Maximum PV voltage in volts")
    min_battery_voltage_v: float = Field(..., alias="Min. battery voltage(V)", ge=0, description="Minimum battery voltage in volts")
    max_battery_voltage_v: float = Field(..., alias="Max. battery voltage(V)", ge=0, description="Maximum battery voltage in volts")
    time_in_bulk_m: int = Field(..., alias="Time in bulk(m)", ge=0, description="Time in bulk charge phase in minutes")
    time_in_absorption_m: int = Field(..., alias="Time in absorption(m)", ge=0, description="Time in absorption phase in minutes")
    time_in_float_m: int = Field(..., alias="Time in float(m)", ge=0, description="Time in float phase in minutes")
    last_error: int = Field(..., alias="Last error", description="Last error code")
    second_last_error: int = Field(..., alias="2nd last error", description="Second last error code")
    third_last_error: int = Field(..., alias="3rd last error", description="Third last error code")
    fourth_last_error: int = Field(..., alias="4th last error", description="Fourth last error code")
